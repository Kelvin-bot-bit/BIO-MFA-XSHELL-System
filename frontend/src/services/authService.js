...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Face verification failures should NOT redirect to login or clear tokens
    ...
    // SPECIAL: Also skip for verify-reset-token and reset-password
    if (
      ...
      if (refreshToken) {
        try {
          ...
          if (refreshResponse.data.success) {
            ...
            // Only redirect for non-face endpoints
            if (!originalRequest?.url?.includes("/verify-face")) {
              handleAuthFailure();
            }
            return Promise.reject(error);
